...
USE_FUSED_MOE = os.environ.get("VLLM_USE_FUSED_MOE", "0") == "1"
if USE_FUSED_MOE:
    from vllm.model_executor.layers.fused_moe.fused_moe_persistent import (
        fused_moe_persistent,
    )
    logger.info("Fused persistent Triton MoE kernel enabled")

USE_ASM_MOE = os.environ.get("VLLM_USE_ASM_MOE", "0") == "1"
if USE_ASM_MOE:
    from vllm.model_executor.layers.fused_moe.custom_moe_triton_kernel import (
        custom_moe_forward,
    )
    logger.info("Custom Triton MoE kernel enabled")
