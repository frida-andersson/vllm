...
class ROCMAiterMLASparseMetadataBuilder(
    ...
    def __init__(
        ...
        # The parent allocates paged_kv_indices sized for
        # max_num_reqs * max_model_len pages (one page per token with
        # kernel_block_size=1). With kernel_block_size=64 that buffer is 64x
        # larger than needed, and our _build_decode override never uses it.
        # Release to save memory (~52 MB per layer).
        self.paged_kv_indices = torch.zeros(1, dtype=torch.int32, device=device)

        # Also release persistent MLA metadata buffers from the parent —
        # our sparse _build_decode doesn't call get_mla_metadata_v1.
        _placeholder = torch.empty(0, dtype=torch.int32, device=device)
        self._mla_work_meta_data = torch.empty(
            0, dtype=torch.uint64, device=device
        )
        self._mla_work_indptr = _placeholder
        self._mla_work_info_set = _placeholder
        self._mla_reduce_indptr = _placeholder
        self._mla_reduce_final_map = _placeholder
        self._mla_reduce_partial_map = _placeholder
    ...
    def _build_decode(
        ...
        # Our forward_mqa only reads block_table and seq_lens from
        # decode metadata. Skip parent's page index expansion and
        # get_mla_metadata_v1 call (which would OOB with block_size=64).
...
class ROCMAiterMLASparseImpl(AiterMLAImpl):
    ...
    def _forward_sparse_mla(
        ...
        mla_kwargs = {}
        if is_fp8_kv:
            mla_kwargs["q_scale"] = q_scale
            mla_kwargs["kv_scale"] = k_scale
