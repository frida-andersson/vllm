...
class MLADualRMSGroupQuantPattern(
    ...
    """
    Fuse the MLA FP8 attention path -- q-latent RMSNorm + FP8 *group*
    (block-scale) quant plus kv-latent RMSNorm -- into AITER's
    ``fused_qk_rmsnorm_group_quant``.

    With a block-scale FP8 ``q_b_proj`` (DeepSeek-V3.2), the earlier
    ``RocmAiterRMSNormQuantFusionPass`` folds the q side into
    ``rocm_aiter_rmsnorm_fp8_group_quant`` and leaves the kv side a plain
    ``vllm_ir.rms_norm``. This pattern matches that asymmetric pair (single
    q consumer)::
    ...
    def replacement(
        ...
        def _replacement(
            ...
            q_c, kv_lora = projected.split([q_dim, kv_dim], dim=-1)
            kv_c, k_pe = kv_lora.split([kv_c_dim, k_pe_dim], dim=-1)
            at = fused_op(q_c, q_weight, kv_c, kv_weight, eps, eps, group_size)
...
class MLADualRMSNormFusionPass(VllmFusionPatternMatcherPass):
    ...
    The FP8 *group*-quant attention path (DeepSeek-V3.2) is also handled via
    :class:`MLADualRMSGroupQuantPattern` (single q consumer), which fuses the
    q-latent RMSNorm + FP8 group quant together with the kv-latent RMSNorm into
    ``fused_mla_dual_rms_norm_group_quant`` backed by aiter's
    ``fused_qk_rmsnorm_group_quant`` HIP kernel.

    The ~16/61 DeepSeek-V3.2 layers where the q latent fans out to a second
    group-quant consumer (the MLA indexer ``wq_b``) are intentionally left
    unfused: the ceiling there is <1% TPOT at the high-ISL operating point and
    the double-consumer pattern did not fire in-server, so it was dropped.
    """
    ...
    def __init__(self, config: VllmConfig) -> None:
        ...
        if check_aiter_fused_qk_rmsnorm_group_quant():
            for epsilon in [1e-5, 1e-6]:
